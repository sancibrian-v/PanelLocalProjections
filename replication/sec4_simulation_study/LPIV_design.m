%%% SIMULATION STUDY: LP-IV DESIGN %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This script runs Monte Carlo simulations to compare the coverage rates of 
% different confidence intervals for panel local projections with
% instrumental variables (LP-IV) using a general dynamic data generating
% process (DGP).
%
...
% runs in parallel using 100 workers (see line 297).
%
% Version: 2024 February 08 - Matlab R2022a
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Set up directories
fig_dir = ['output' filesep 'figures' filesep main_name filesep];
...
% Specify data generating process (main equation)
rho_s             = 0.5;
n_lambda          = 10;
...
% Specify data generating process (first stage and instrument)
par_beta_endo          = struct();
par_beta_endo.mean_AR  = 0.3;
par_beta_endo.nobs_AR  = Inf;
par_beta_endo.mean_MA  = -0.2;
par_beta_endo.nobs_MA  = Inf;
beta_endo              = draw_poly(par_beta_endo, L_bar+1);
beta_endo              = beta_endo./beta_endo(1);
par_gamma_endo         = struct();
par_gamma_endo.mean_AR = [0.4, 0.2];
par_gamma_endo.nobs_AR = Inf;
par_gamma_endo.mean_MA = [0.1, -0.1];
par_gamma_endo.nobs_MA = Inf;
gamma_endo             = draw_poly(par_gamma_endo, L_bar+1);
gamma_endo             = gamma_endo./sqrt(sum(gamma_endo.^2));
a_IV                   = 10;
...
LPIV_mean = NaN(H_max+1, 2, n_MC);
LPIV_proj = NaN(H_max+1, 2, n_MC);

% Preallocate confidence intervals
SE_LP_mean   = NaN(H_max+1, n_CI, n_MC);
df_LP_mean   = NaN(H_max+1, n_CI, n_MC);
SE_LP_proj   = NaN(H_max+1, n_CI, n_MC);
df_LP_proj   = NaN(H_max+1, n_CI, n_MC);
SE_LPIV_mean = NaN(H_max+1, n_CI, n_MC);
df_LPIV_mean = NaN(H_max+1, n_CI, n_MC);
SE_LPIV_proj = NaN(H_max+1, n_CI, n_MC);
df_LPIV_proj = NaN(H_max+1, n_CI, n_MC);
...
parfor i_MC = 1:n_MC
    ...
    % Simulate endogenous regressor
    X_endo = NaN(T, 1);
    for t = 1:T
        X_endo(t) = beta_endo * X(t+(L_bar:(-1):0), 1) + gamma_endo * Z(t+(L_bar:(-1):0), 1);
    end
    ...
    X_IV = NaN(T, 1);
    for t = 1:T
        X_IV(t) = a_IV * X(t+L_bar, 1) + U_IV(t, 1);
    end
    ...
    LPIV_mean_par = NaN(H_max+1, 2);
    LPIV_proj_par = NaN(H_max+1, 2);
    
    % Set temporary storage for confidence intervals
    SE_LP_mean_par   = NaN(H_max+1, n_CI);
    df_LP_mean_par   = NaN(H_max+1, n_CI);
    SE_LP_proj_par   = NaN(H_max+1, n_CI);
    df_LP_proj_par   = NaN(H_max+1, n_CI);
    SE_LPIV_mean_par = NaN(H_max+1, n_CI);
    df_LPIV_mean_par = NaN(H_max+1, n_CI);
    SE_LPIV_proj_par = NaN(H_max+1, n_CI);
    df_LPIV_proj_par = NaN(H_max+1, n_CI);
    ...
    for h = 0:H_max
        ...
        SE_LP_mean_par(h+1, 1) = sqrt(max(0, b_var(1)));
        df_LP_mean_par(h+1, 1) = Inf;

        % Compute two-way standard error
        Xv_var = (Xv_i')*Xv_i + (Xv_t')*Xv_t - (Xv_it')*Xv_it;
        b_var  = pinv(XX)*Xv_var*pinv(XX);
        SE_LP_mean_par(h+1, 2) = sqrt(max(0, b_var(1)));
        df_LP_mean_par(h+1, 2) = Inf;
        ...
        b_var  = pinv(XX)*Xv_var*pinv(XX);
        SE_LP_mean_par(h+1, 3) = sqrt(max(0, b_var(1)));
        df_LP_mean_par(h+1, 3) = Inf;

        % Compute t-HR standard error
        Xv_var = (Xv_t')*Xv_t;
        b_var  = pinv(XX)*Xv_var*pinv(XX);
        SE_LP_mean_par(h+1, 4) = sqrt(max(0, b_var(1)));
        df_LP_mean_par(h+1, 4) = Inf;
        ...
        b_var     = pinv(XX)*Xv_var*pinv(XX);
        SE_LP_mean_par(h+1, 6) = sqrt(max(0, b_var(1)));
        df_LP_mean_par(h+1, 6) = n_EWC;
        ...
        x     = repmat(X_endo_sim((n_lag+1):(T-h)), [1, N]);
        W     = zeros(T_est, N, n_W);
        for i_lag = 1:n_lag
            for i_Y = 1:n_Y
                W(:, :, (n_Y+1)*(i_lag-1)+i_Y) = reshape(Y_sim(i_Y, ((n_lag+1):(T-h))-i_lag, :), [T_est, N]);
            end
            W(:, :, (n_Y+1)*i_lag) = repmat(X_endo_sim(((n_lag+1):(T-h))-i_lag), [1, N]);
        end
        y     = y - mean(y, 1);
        x     = x - mean(x, 1);
        W     = W - mean(W, 1);
        ...
        if (T > 100)
            Xv_var = (Xv_t')*Xv_t;
            b_var  = pinv(XX)*Xv_var*pinv(XX);
            SE_LP_mean_par(h+1, 5) = sqrt(max(0, b_var(1)));
            df_LP_mean_par(h+1, 5) = Inf;
        else
            ...
            lam0   = eig(G0'*G0);
            df_LP_mean_par(h+1, 5) = (sum(lam0))^2/(sum(lam0.^2));
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        x     = zeros(T_est, N, n_S);
        for i_S = 1:n_S
            x(:, :, i_S) = X_endo_sim(1:(T-h)) .* s_sim(i_S, :);
        end
        ...
        Xv_var = (Xv_i')*Xv_i;
        b_var  = pinv(XX)*Xv_var*pinv(XX);
        SE_LP_proj_par(h+1, 1) = sqrt(max(0, b_var(1)));
        df_LP_proj_par(h+1, 1) = Inf;

        % Compute two-way standard error
        Xv_var = (Xv_i')*Xv_i + (Xv_t')*Xv_t - (Xv_it')*Xv_it;
        b_var  = pinv(XX)*Xv_var*pinv(XX);
        SE_LP_proj_par(h+1, 2) = sqrt(max(0, b_var(1)));
        df_LP_proj_par(h+1, 2) = Inf;
        ...
        b_var  = pinv(XX)*Xv_var*pinv(XX);
        SE_LP_proj_par(h+1, 3) = sqrt(max(0, b_var(1)));
        df_LP_proj_par(h+1, 3) = Inf;

        % Compute t-HR standard error
        Xv_var = (Xv_t')*Xv_t;
        b_var  = pinv(XX)*Xv_var*pinv(XX);
        SE_LP_proj_par(h+1, 4) = sqrt(max(0, b_var(1)));
        df_LP_proj_par(h+1, 4) = Inf;        
        ...
        Xv_var    = ((Xv_t')*(cos_trans')*cos_trans*Xv_t)/n_EWC;
        b_var     = pinv(XX)*Xv_var*pinv(XX);
        SE_LP_proj_par(h+1, 6) = sqrt(max(0, b_var(1)));
        df_LP_proj_par(h+1, 6) = n_EWC;
        ...
        x     = zeros(T_est, N, n_S);
        for i_S = 1:n_S
            x(:, :, i_S) = X_endo_sim((n_lag+1):(T-h)) .* s_sim(i_S, :);
        end
        W     = zeros(T_est, N, n_W);
        for i_lag = 1:n_lag
            for i_Y = 1:n_Y
            ...
            for i_S = 1:n_S
                W(:, :, (n_Y+n_S)*(i_lag-1)+n_Y+i_S) = X_endo_sim(((n_lag+1):(T-h))-i_lag) .* s_sim(i_S, :);
            end
        end        
        y     = y - mean(y, 1) - mean(y, 2) + mean(y(:)); 
        x     = x - mean(x, 1) - mean(x, 2) + reshape(mean(reshape(x, T_est*N, n_S), 1), [1, 1, n_S]);
        ...
        if (T > 100)
            Xv_var = (Xv_t')*Xv_t;
            b_var  = pinv(XX)*Xv_var*pinv(XX);
            SE_LP_proj_par(h+1, 5) = sqrt(max(0, b_var(1)));
            df_LP_proj_par(h+1, 5) = Inf;
        else
            ...
            for t = 1:T_est, G0(:, t) = P0(:, t)*X_t(t, :)*XX0(:, 1)/sqrt(P0(t, t)); end
            lam0   = eig(G0'*G0);
            df_LP_proj_par(h+1, 5) = (sum(lam0))^2/(sum(lam0.^2));
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
        %%% no lags
        % Compute pooled LPIV estimator (unit FE)
        T_est = T-h;
        n_W   = 0;
        y     = reshape(Y_sim(1, (h+1):T, :), [T_est, N]);
        x     = repmat(X_endo_sim(1:(T-h)), [1, N]);
        z     = repmat(X_IV_sim(1:(T-h)), [1, N]);
        y     = y - mean(y, 1);
        x     = x - mean(x, 1);
        z     = z - mean(z, 1);
        X     = x(:);
        Z     = z(:);
        b     = pinv((Z')*X)*((Z')*y(:));
        LPIV_mean_par(h+1, 1) = b(1);

        % Compute score and hessian
        n_X   = 1+n_W;
        Xv    = reshape(Z .* (y(:) - X*b), [T_est, N, n_X]);
        Xv_i  = reshape(sum(Xv, 1), [N, n_X]);
        Xv_t  = reshape(sum(Xv, 2), [T_est, n_X]);
        Xv_it = reshape(Xv, [T_est*N, n_X]);
        XX    = (Z')*X;
        ...
        Xv_var = (Xv_i')*Xv_i;
        b_var  = pinv(XX)*Xv_var*pinv(XX);
        SE_LPIV_mean_par(h+1, 1) = sqrt(max(0, b_var(1)));
        df_LPIV_mean_par(h+1, 1) = Inf;

        % Compute two-way standard error
        Xv_var = (Xv_i')*Xv_i + (Xv_t')*Xv_t - (Xv_it')*Xv_it;
        b_var  = pinv(XX)*Xv_var*pinv(XX);
        SE_LPIV_mean_par(h+1, 2) = sqrt(max(0, b_var(1)));
        df_LPIV_mean_par(h+1, 2) = Inf;
        ...
        b_var  = pinv(XX)*Xv_var*pinv(XX);
        SE_LPIV_mean_par(h+1, 3) = sqrt(max(0, b_var(1)));
        df_LPIV_mean_par(h+1, 3) = Inf;

        % Compute t-HR standard error
        Xv_var = (Xv_t')*Xv_t;
        b_var  = pinv(XX)*Xv_var*pinv(XX);
        SE_LPIV_mean_par(h+1, 4) = sqrt(max(0, b_var(1)));
        df_LPIV_mean_par(h+1, 4) = Inf;
        ...
        Xv_var    = ((Xv_t')*(cos_trans')*cos_trans*Xv_t)/n_EWC;
        b_var     = pinv(XX)*Xv_var*pinv(XX);
        SE_LPIV_mean_par(h+1, 6) = sqrt(max(0, b_var(1)));
        df_LPIV_mean_par(h+1, 6) = n_EWC;

        %%% lag augmentation
        % Compute LPIV estimator (1, unit FE)
        n_lag = min(h, ceil((T-h)^(1/3)));
        T_est = T-h-n_lag;
        n_W   = (n_Y+1)*n_lag;
        y     = reshape(Y_sim(1, (h+n_lag+1):T, :), [T_est, N] );
        x     = repmat(X_endo_sim((n_lag+1):(T-h)), [1, N]);
        z     = repmat(X_IV_sim((n_lag+1):(T-h)), [1, N]);
        W     = zeros(T_est, N, n_W);
        for i_lag = 1:n_lag
            for i_Y = 1:n_Y
                W(:, :, (n_Y+1)*(i_lag-1)+i_Y) = reshape(Y_sim(i_Y, ((n_lag+1):(T-h))-i_lag, :), [T_est, N]);
        ...
        W_z   = zeros(T_est, N, n_W);
        for i_lag = 1:n_lag
            for i_Y = 1:n_Y
                W_z(:, :, (n_Y+1)*(i_lag-1)+i_Y) = reshape(Y_sim(i_Y, ((n_lag+1):(T-h))-i_lag, :), [T_est, N]);
            end
            W_z(:, :, (n_Y+1)*i_lag) = repmat(X_IV_sim(((n_lag+1):(T-h))-i_lag), [1, N]);
        end
        y     = y - mean(y, 1);
        x     = x - mean(x, 1);
        z     = z - mean(z, 1);
        ...
        W_z   = W_z - mean(W_z, 1);
        X     = [x(:), reshape(W, [T_est*N, n_W])];
        Z     = [z(:), reshape(W_z, [T_est*N, n_W])];
        b     = pinv((Z')*X)*((Z')*y(:));
        LPIV_mean_par(h+1, 2) = b(1);
        ...
        if (T > 100)
            Xv_var = (Xv_t')*Xv_t;
            b_var  = pinv(XX)*Xv_var*pinv(XX);
            SE_LPIV_mean_par(h+1, 5) = sqrt(max(0, b_var(1)));
            df_LPIV_mean_par(h+1, 5) = Inf;
        else
            X_tmp  = reshape(X, [T_est, N, n_X]);
            X_t    = NaN(T_est, n_X);
            for t = 1:T_est
            ...
            for t = 1:T_est, G0(:, t) = P0(:, t)*X_t(t, :)*XX0(:, 1)/sqrt(P0(t, t)); end
            lam0   = eig(G0'*G0);
            df_LPIV_mean_par(h+1, 5) = (sum(lam0))^2/(sum(lam0.^2));
        end        
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
        %%% no lag augmentation
        % Compute LPIV estimator (s_i, unit FE, time FE)
        T_est = T-h;
        n_W   = 0;
        y     = reshape(Y_sim(1, (h+1):T, :), [T_est, N] );
        x     = zeros(T_est, N, n_S);
        ...
        z     = zeros(T_est, N, n_S);
        for i_S = 1:n_S
            z(:, :, i_S) = X_IV_sim(1:(T-h)) .* s_sim(i_S, :);
        end
        y     = y - mean(y, 1) - mean(y, 2) + mean(y(:)); 
        x     = x - mean(x, 1) - mean(x, 2) + reshape(mean(reshape(x, T_est*N, n_S), 1), [1, 1, n_S]);
        z     = z - mean(z, 1) - mean(z, 2) + reshape(mean(reshape(z, T_est*N, n_S), 1), [1, 1, n_S]);
        X     = reshape(x, [T_est*N, n_S]);
        Z     = reshape(z, [T_est*N, n_S]);
        b     = pinv((Z')*X)*((Z')*y(:));
        LPIV_proj_par(h+1, 1) = b(1);
        ...
        Xv_var = (Xv_i')*Xv_i;
        b_var  = pinv(XX)*Xv_var*pinv(XX);
        SE_LPIV_proj_par(h+1, 1) = sqrt(max(0, b_var(1)));
        df_LPIV_proj_par(h+1, 1) = Inf;

        % Compute two-way standard error
        Xv_var = (Xv_i')*Xv_i + (Xv_t')*Xv_t - (Xv_it')*Xv_it;
        b_var  = pinv(XX)*Xv_var*pinv(XX);
        SE_LPIV_proj_par(h+1, 2) = sqrt(max(0, b_var(1)));
        df_LPIV_proj_par(h+1, 2) = Inf;
        ...
        b_var  = pinv(XX)*Xv_var*pinv(XX);
        SE_LPIV_proj_par(h+1, 3) = sqrt(max(0, b_var(1)));
        df_LPIV_proj_par(h+1, 3) = Inf;

        % Compute t-HR standard error
        Xv_var = (Xv_t')*Xv_t;
        b_var  = pinv(XX)*Xv_var*pinv(XX);
        SE_LPIV_proj_par(h+1, 4) = sqrt(max(0, b_var(1)));
        df_LPIV_proj_par(h+1, 4) = Inf;        
        ...
        Xv_var    = ((Xv_t')*(cos_trans')*cos_trans*Xv_t)/n_EWC;
        b_var     = pinv(XX)*Xv_var*pinv(XX);
        SE_LPIV_proj_par(h+1, 6) = sqrt(max(0, b_var(1)));
        df_LPIV_proj_par(h+1, 6) = n_EWC;
        ...
        z     = zeros(T_est, N, n_S);
        for i_S = 1:n_S
            z(:, :, i_S) = X_IV_sim((n_lag+1):(T-h)) .* s_sim(i_S, :);
        end
        W     = zeros(T_est, N, n_W);
        for i_lag = 1:n_lag
            for i_Y = 1:n_Y
        ...
        for i_lag = 1:n_lag
            for i_Y = 1:n_Y
                W_z(:, :, (n_Y+n_S)*(i_lag-1)+i_Y) = reshape(Y_sim(i_Y, ((n_lag+1):(T-h))-i_lag, :), [T_est, N]);
            end
            for i_S = 1:n_S
                W_z(:, :, (n_Y+n_S)*(i_lag-1)+n_Y+i_S) = X_IV_sim(((n_lag+1):(T-h))-i_lag) .* s_sim(i_S, :);
            end
        end
        y     = y - mean(y, 1) - mean(y, 2) + mean(y(:)); 
        x     = x - mean(x, 1) - mean(x, 2) + reshape(mean(reshape(x, T_est*N, n_S), 1), [1, 1, n_S]);
        z     = z - mean(z, 1) - mean(z, 2) + reshape(mean(reshape(z, T_est*N, n_S), 1), [1, 1, n_S]);
        W     = W - mean(W, 1) - mean(W, 2) + reshape(mean(reshape(W, T_est*N, n_W), 1), [1, 1, n_W]);
        W_z   = W_z - mean(W_z, 1) - mean(W_z, 2) + reshape(mean(reshape(W_z, T_est*N, n_W), 1), [1, 1, n_W]);
        X     = [reshape(x, [T_est*N, n_S]), reshape(W, [T_est*N, n_W])];
        Z     = [reshape(z, [T_est*N, n_S]), reshape(W_z, [T_est*N, n_W])];
        b     = pinv((Z')*X)*((Z')*y(:));
        LPIV_proj_par(h+1, 2) = b(1);
        ...
        if (T > 100)
            Xv_var = (Xv_t')*Xv_t;
            b_var  = pinv(XX)*Xv_var*pinv(XX);
            SE_LPIV_proj_par(h+1, 5) = sqrt(max(0, b_var(1)));
            df_LPIV_proj_par(h+1, 5) = Inf;
        else
            X_tmp  = permute(reshape(X, [T_est, N, n_X]), [2, 3, 1]);
            X_t    = NaN(T_est, n_X);
            for t = 1:T_est
            ...
            for t = 1:T_est, G0(:, t) = P0(:, t)*X_t(t, :)*XX0(:, 1)/sqrt(P0(t, t)); end
            lam0   = eig(G0'*G0);
            df_LPIV_proj_par(h+1, 5) = (sum(lam0))^2/(sum(lam0.^2));
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    LPIV_mean(:, :, i_MC) = LPIV_mean_par;
    LPIV_proj(:, :, i_MC) = LPIV_proj_par;
    
    % Save confidence intervals
    SE_LP_mean(:, :, i_MC)   = SE_LP_mean_par;
    df_LP_mean(:, :, i_MC)   = df_LP_mean_par;
    SE_LP_proj(:, :, i_MC)   = SE_LP_proj_par;
    df_LP_proj(:, :, i_MC)   = df_LP_proj_par;
    SE_LPIV_mean(:, :, i_MC) = SE_LPIV_mean_par;
    df_LPIV_mean(:, :, i_MC) = df_LPIV_mean_par;
    SE_LPIV_proj(:, :, i_MC) = SE_LPIV_proj_par;
    df_LPIV_proj(:, :, i_MC) = df_LPIV_proj_par;
...
sim_obj.LPIV_mean     = LPIV_mean;
sim_obj.LP_proj       = LP_proj;
sim_obj.LPIV_proj     = LPIV_proj;
sim_obj.CI_names      = CI_names;
sim_obj.estimator_id  = estimator_id;
sim_obj.SE_LP_mean    = SE_LP_mean;
sim_obj.df_LP_mean    = df_LP_mean;
sim_obj.SE_LPIV_mean  = SE_LPIV_mean;
sim_obj.df_LPIV_mean  = df_LPIV_mean;
sim_obj.SE_LP_proj    = SE_LP_proj;
sim_obj.df_LP_proj    = df_LP_proj;
sim_obj.SE_LPIV_proj  = SE_LPIV_proj;
sim_obj.df_LPIV_proj  = df_LPIV_proj;
...
LPIV_mean     = sim_obj.LPIV_mean;
LP_proj       = sim_obj.LP_proj;
LPIV_proj     = sim_obj.LPIV_proj;
CI_names      = sim_obj.CI_names;
n_CI          = length(CI_names);
estimator_id  = sim_obj.estimator_id;
SE_LP_mean    = sim_obj.SE_LP_mean;
df_LP_mean    = sim_obj.df_LP_mean;
SE_LPIV_mean  = sim_obj.SE_LPIV_mean;
df_LPIV_mean  = sim_obj.df_LPIV_mean;
SE_LP_proj    = sim_obj.SE_LP_proj;
df_LP_proj    = sim_obj.df_LP_proj;
SE_LPIV_proj  = sim_obj.SE_LPIV_proj;
df_LPIV_proj  = sim_obj.df_LPIV_proj;

% Set significance level and critical values
signif = 0.90;

% Preallocate coverage and expected length
cov_LP_mean   = NaN(n_CI, H_max+1);
len_LP_mean   = NaN(n_CI, H_max+1);
cov_LP_proj   = NaN(n_CI, H_max+1);
len_LP_proj   = NaN(n_CI, H_max+1);
cov_LPIV_mean = NaN(n_CI, H_max+1);
len_LPIV_mean = NaN(n_CI, H_max+1);
cov_LPIV_proj = NaN(n_CI, H_max+1);
len_LPIV_proj = NaN(n_CI, H_max+1);

% Compute summaries for confidence intervals for LP (1, unit FE)
for h = 0:H_max
    for i_CI = 1:n_CI
        LP_tmp   = reshape( LP_mean(h+1, estimator_id(i_CI), :), [1, n_MC]);
        SE_tmp   = reshape( SE_LP_mean(h+1, i_CI, :), [1, n_MC]);
        cv_tmp   = tinv(1-(1-signif)/2, reshape( df_LP_mean(h+1, i_CI, :), [1, n_MC]));
        low_tmp  = LP_tmp - SE_tmp .* cv_tmp;
        upp_tmp  = LP_tmp + SE_tmp .* cv_tmp;
        true_tmp = estimand_mean(h+1, :);
        cov_LP_mean(i_CI, h+1) = mean( (low_tmp <= true_tmp) & (true_tmp <= upp_tmp) );
        len_LP_mean(i_CI, h+1) = median(upp_tmp - low_tmp);
    end
end
...
for h = 0:H_max
    for i_CI = 1:n_CI
        ...
        SE_tmp   = reshape( SE_LP_proj(h+1, i_CI, :), [1, n_MC]);
        cv_tmp   = tinv(1-(1-signif)/2, reshape( df_LP_proj(h+1, i_CI, :), [1, n_MC]));
        low_tmp  = LP_tmp - SE_tmp .* cv_tmp;
        upp_tmp  = LP_tmp + SE_tmp .* cv_tmp;
        true_tmp = estimand_proj(h+1, :);
        cov_LP_proj(i_CI, h+1) = mean( (low_tmp <= true_tmp) & (true_tmp <= upp_tmp) );
        len_LP_proj(i_CI, h+1) = median(upp_tmp - low_tmp);
    end
end

% Compute summaries for confidence intervals for LPIV (1, unit FE)
for h = 0:H_max
    for i_CI = 1:n_CI
        LP_tmp   = reshape( LPIV_mean(h+1, estimator_id(i_CI), :), [1, n_MC]);
        SE_tmp   = reshape( SE_LPIV_mean(h+1, i_CI, :), [1, n_MC]);
        cv_tmp   = tinv(1-(1-signif)/2, reshape( df_LPIV_mean(h+1, i_CI, :), [1, n_MC]));
        low_tmp  = LP_tmp - SE_tmp .* cv_tmp;
        upp_tmp  = LP_tmp + SE_tmp .* cv_tmp;
        true_tmp = estimand_mean(h+1, :);
        cov_LPIV_mean(i_CI, h+1) = mean( (low_tmp <= true_tmp) & (true_tmp <= upp_tmp) );
        len_LPIV_mean(i_CI, h+1) = median(upp_tmp - low_tmp);
    end
end

% Compute summaries for confidence intervals for LPIV (s_i, unit FE, time FE)
for h = 0:H_max
    for i_CI = 1:n_CI
        LP_tmp   = reshape( LPIV_proj(h+1, estimator_id(i_CI), :), [1, n_MC]);
        SE_tmp   = reshape( SE_LPIV_proj(h+1, i_CI, :), [1, n_MC]);
        cv_tmp   = tinv(1-(1-signif)/2, reshape( df_LPIV_proj(h+1, i_CI, :), [1, n_MC]));
        low_tmp  = LP_tmp - SE_tmp .* cv_tmp;
        upp_tmp  = LP_tmp + SE_tmp .* cv_tmp;
        true_tmp = estimand_proj(h+1, :);
        cov_LPIV_proj(i_CI, h+1) = mean( (low_tmp <= true_tmp) & (true_tmp <= upp_tmp) );
        len_LPIV_proj(i_CI, h+1) = median(upp_tmp - low_tmp);
    end
end

% Save coverage rates and expected length
sim_obj.cov_LP_mean   = cov_LP_mean;
sim_obj.len_LP_mean   = len_LP_mean;
sim_obj.cov_LP_proj   = cov_LP_proj;
sim_obj.len_LP_proj   = len_LP_proj;
sim_obj.cov_LPIV_mean = cov_LPIV_mean;
sim_obj.cov_LPIV_proj = cov_LPIV_proj;
sim_obj.len_LPIV_mean = len_LPIV_mean;
sim_obj.len_LPIV_proj = len_LPIV_proj;
save([mat_dir design_str '.mat'], '-struct', 'sim_obj')
...
tab_summary = array2table([reshape(cov_LP_mean, [n_CI, H_max+1]), len_LP_mean], 'RowNames', CI_names, 'VariableNames', var_names);
disp(tab_summary)
writetable(tab_summary, [tab_dir design_str '.xlsx'], 'Sheet', 'LP-mean', 'WriteRowNames', true, 'WriteVariableNames', true)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Tabulate summaries for LP (s_i, unit FE, time FE) %%%%%%%%%%%%%%%%%%%%%%%
tab_summary = array2table([reshape(cov_LP_proj, [n_CI, H_max+1]), len_LP_proj], 'RowNames', CI_names, 'VariableNames', var_names);
disp(tab_summary)
writetable(tab_summary, [tab_dir design_str '.xlsx'], 'Sheet', 'LP-proj', 'WriteRowNames', true, 'WriteVariableNames', true)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Tabulate summaries for LPIV (1, unit FE) %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
tab_summary = array2table([reshape(cov_LPIV_mean, [n_CI, H_max+1]), len_LPIV_mean], 'RowNames', CI_names, 'VariableNames', var_names);
disp(tab_summary)
writetable(tab_summary, [tab_dir design_str '.xlsx'], 'Sheet', 'LPIV-mean', 'WriteRowNames', true, 'WriteVariableNames', true)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Tabulate summaries for LPIV (s_i, unit FE, time FE) %%%%%%%%%%%%%%%%%%%%%
tab_summary = array2table([reshape(cov_LPIV_proj, [n_CI, H_max+1]), len_LPIV_proj], 'RowNames', CI_names, 'VariableNames', var_names);
disp(tab_summary)
writetable(tab_summary, [tab_dir design_str '.xlsx'], 'Sheet', 'LPIV-proj', 'WriteRowNames', true, 'WriteVariableNames', true)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Plot sampling distribution of LPIV (1, unit FE, no lags) %%%%%%%%%%%%%%%%
est_true  = mean(estimand_mean, 2);
est_mean  = mean(LPIV_mean(:, 1, :), 3);
est_quant = quantile(LPIV_mean(:, 1, :), quants, 3);
horizons = (0:H_max)';
fig0     = figure();
ax0      = axes();
for j = 1:(n_quant-1)/2
...
% Tune and save figure
set(fig0, figsize{:});
print(fig0, [fig_dir_local 'sampling_distribution-LPIV-mean'], '-dpdf', '-vector')
ylim_bias = get(ax0, 'ylim');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Plot sampling distribution of LPIV (1, unit FE, lags) %%%%%%%%%%%%%%%%%%%
est_true  = mean(estimand_mean, 2);
est_mean  = mean(LPIV_mean(:, 2, :), 3);
est_quant = quantile(LPIV_mean(:, 2, :), quants, 3);
horizons = (0:H_max)';
fig0     = figure();
ax0      = axes();
for j = 1:(n_quant-1)/2
...
% Tune and save figure
set(fig0, figsize{:});
print(fig0, [fig_dir_local 'sampling_distribution-LALPIV-mean'], '-dpdf', '-vector')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Plot sampling distribution of LPIV (s_i, unit FE, time FE, no lags) %%%%%
est_true  = mean(estimand_proj, 2);
est_mean  = mean(LPIV_proj(:, 1, :), 3);
est_quant = quantile(LPIV_proj(:, 1, :), quants, 3);
horizons = (0:H_max)';
fig0     = figure();
ax0      = axes();
for j = 1:(n_quant-1)/2
...
% Tune and save figure
set(fig0, figsize{:});
print(fig0, [fig_dir_local 'sampling_distribution-LPIV-proj'], '-dpdf', '-vector')
ylim_bias = get(ax0, 'ylim');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Plot sampling distribution of LPIV (s_i, unit FE, time FE, lags) %%%%%%%%
est_true  = mean(estimand_proj, 2);
est_mean  = mean(LPIV_proj(:, 2, :), 3);
est_quant = quantile(LPIV_proj(:, 2, :), quants, 3);
horizons = (0:H_max)';
fig0     = figure();
ax0      = axes();
for j = 1:(n_quant-1)/2
...
% Tune and save figure
set(fig0, figsize{:});
print(fig0, [fig_dir_local 'sampling_distribution-LALPIV-proj'], '-dpdf', '-vector')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
fig0     = figure();
ax0      = axes();
plot0    = plot(ax0, horizons, cov_LP_mean');
xlabel(ax0, 'h')
...
% Tune and save figure
set(fig0, figsize{:});
print(fig0, [fig_dir_local 'cov-LP-mean'], '-dpdf', '-vector')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
fig0     = figure();
ax0      = axes();
plot0    = plot(ax0, horizons, cov_LP_proj');
xlabel(ax0, 'h')
ylabel(ax0, ['Coverage rate of ' num2str(100*signif) '% CI'])
...
% Tune and save figure
set(fig0, figsize{:});
print(fig0, [fig_dir_local 'cov-LP-proj'], '-dpdf', '-vector')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Plot coverage rates for LPIV (1, unit FE) %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
horizons = (0:H_max)';
fig0     = figure();
ax0      = axes();
plot0    = plot(ax0, horizons, cov_LPIV_mean');
xlabel(ax0, 'h')
ylabel(ax0, ['Coverage rate of ' num2str(100*signif) '% CI'])
...
% Tune and save figure
set(fig0, figsize{:});
print(fig0, [fig_dir_local 'cov-LPIV-mean'], '-dpdf', '-vector')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Plot coverage rates for LPIV (s_i, unit FE, time FE) %%%%%%%%%%%%%%%%%%%%
horizons = (0:H_max)';
fig0     = figure();
ax0      = axes();
plot0    = plot(ax0, horizons, cov_LPIV_proj');
xlabel(ax0, 'h')
ylabel(ax0, ['Coverage rate of ' num2str(100*signif) '% CI'])
...
% Tune and save figure
set(fig0, figsize{:});
print(fig0, [fig_dir_local 'cov-LPIV-proj'], '-dpdf', '-vector')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
fig0     = figure();
ax0      = axes();
plot0    = plot(ax0, horizons, len_LP_mean');
xlabel(ax0, 'h')
...
set(ax0, 'LooseInset', get(ax0, 'TightInset'))
xlim(ax0, [horizons(1), horizons(H_max)])
ylim(ax0, [0, 1.5*max(len_LP_mean(:))])
set(ax0, font{:})
grid(ax0, 'on')
...
% Tune and save figure
set(fig0, figsize{:});
print(fig0, [fig_dir_local 'len-LP-mean'], '-dpdf', '-vector')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
fig0     = figure();
ax0      = axes();
plot0    = plot(ax0, horizons, len_LP_proj');
xlabel(ax0, 'h')
ylabel(ax0, ['Length of ' num2str(100*signif) '% CI'])
...
set(ax0, 'LooseInset', get(ax0, 'TightInset'))
xlim(ax0, [horizons(1), horizons(H_max)])
ylim(ax0, [0, 1.5*max(len_LP_proj(:))])
set(ax0, font{:})
grid(ax0, 'on')
...
% Tune and save figure
set(fig0, figsize{:});
print(fig0, [fig_dir_local 'len-LP-proj'], '-dpdf', '-vector')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Plot length for LPIV (1, unit FE) %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
horizons = (0:H_max)';
fig0     = figure();
ax0      = axes();
plot0    = plot(ax0, horizons, len_LPIV_mean');
xlabel(ax0, 'h')
ylabel(ax0, ['Length of ' num2str(100*signif) '% CI'])
...
set(ax0, 'LooseInset', get(ax0, 'TightInset'))
xlim(ax0, [horizons(1), horizons(H_max)])
ylim(ax0, [0, 1.5*max(len_LPIV_mean(:))])
set(ax0, font{:})
grid(ax0, 'on')
...
% Tune and save figure
set(fig0, figsize{:});
print(fig0, [fig_dir_local 'len-LPIV-mean'], '-dpdf', '-vector')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Plot length for LPIV (s_i, unit FE, time FE) %%%%%%%%%%%%%%%%%%%%%%%%%%%%
horizons = (0:H_max)';
fig0     = figure();
ax0      = axes();
plot0    = plot(ax0, horizons, len_LPIV_proj');
xlabel(ax0, 'h')
ylabel(ax0, ['Length of ' num2str(100*signif) '% CI'])
...
set(ax0, 'LooseInset', get(ax0, 'TightInset'))
xlim(ax0, [horizons(1), horizons(H_max)])
ylim(ax0, [0, 1.5*max(len_LPIV_proj(:))])
set(ax0, font{:})
grid(ax0, 'on')
...
% Tune and save figure
set(fig0, figsize{:});
print(fig0, [fig_dir_local 'len-LPIV-proj'], '-dpdf', '-vector')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end
