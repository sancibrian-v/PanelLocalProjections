%%% SIMULATION STUDY: VAR DESIGN %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This script runs Monte Carlo simulations to compare the coverage rates of 
% different confidence intervals for panel local projections (LPs) using a 
% panel vector autoregressive (VAR) data generating process (DGP).
%
...
rho_s         = 0.5;
n_lambda      = 10;
Lambda_A_AR   = [1-5/T; 0.5];
Lambda_B_MA   = [0.8, -0.5];
p_Y           = length(Lambda_A_AR);
par_A         = struct();
par_A.mean_AR = Lambda_A_AR;
par_A.nobs_AR = Inf;
par_B.mean_MA = Lambda_B_MA;
par_B.nobs_MA = n_lambda;
...
for i_Y = 1:n_Y
    for i = 1:n_dgp
        [~, coefs_A_dgp]    = draw_poly(par_A);
        [~, ~, coefs_B_dgp] = draw_poly(par_B);
        beta_tmp  = NaN(L_bar+1, 1);
        n_A       = length(coefs_A_dgp);
        n_B       = length(coefs_B_dgp);
        AR_aux    = [coefs_A_dgp; eye(n_A-1), zeros(n_A-1, 1)];
        MA_aux    = (1/sqrt(1+sum(coefs_B_dgp.^2)))*[1, coefs_B_dgp];
        IRF_aux   = zeros(n_A, 1);
        for i_lag = 1:(n_B+1)
            IRF_aux         = AR_aux * IRF_aux + [MA_aux(i_lag); zeros(n_A-1, 1)];
            beta_tmp(i_lag) = IRF_aux(1);
        end
        for i_lag = (n_B+2):(L_bar+1)
            IRF_aux         = AR_aux * IRF_aux;
            beta_tmp(i_lag) = IRF_aux(1);
        end
        gamma_tmp = NaN(L_bar+1, 1);
        delta_tmp = NaN(L_bar+1, 1);
        n_B       = 0;
        MA_aux    = 1;
        IRF_aux   = zeros(n_A, 1);
        for i_lag = 1:(n_B+1)
            IRF_aux          = AR_aux * IRF_aux + [MA_aux(i_lag); zeros(n_A-1, 1)];
            gamma_tmp(i_lag) = IRF_aux(1);
            delta_tmp(i_lag) = IRF_aux(1);
        end
        for i_lag = (n_B+2):(L_bar+1)
            IRF_aux          = AR_aux * IRF_aux;
            gamma_tmp(i_lag) = IRF_aux(1);
...
parfor i_MC = 1:n_MC
    ...
    % Simulate heterogeneous parameters
    m = randn(n_Y, N);
    A = zeros(n_Y, n_Y, p_Y, N);
    B = zeros(n_Y, 1+p_Y, N);
    C = zeros(n_Y, N);
    D = zeros(n_Y, n_Y, N);
    for i_Y = 1:n_Y
        for i = 1:N
            [~, coefs_A]      = draw_poly(par_A);
            [~, ~, coefs_B]   = draw_poly(par_B);
            A(i_Y, i_Y, :, i) = coefs_A;
            B(i_Y, :, i)      = (s(1, i)/sqrt(1+sum(coefs_B.^2)))*[1, coefs_B];
            C(i_Y, i)         = s(n_S+1, i);
            D(i_Y, i_Y, i)    = s(n_S+2, i);
        end
    end
    ...
    Y = NaN(n_Y, T, N);
    for i = 1:N
        Y_aux = zeros(n_Y*p_Y, 1);
        for t = (p_Y+1):(L_bar+T)
            X_aux = X(t:(-1):(t-p_Y));
            Y_t   = m(:, i) + reshape(A(:, :, :, i), [n_Y, n_Y*p_Y]) * Y_aux + B(:, :, i) * X_aux + C(:, i) * Z(t) ...
                  + kappa * D(:, :, i) * U(:, t, i);
            Y_aux = [Y_t; Y_aux(1:(n_Y*(p_Y-1)), 1)];
            if (t > L_bar)
                Y(:, t-L_bar, i) = Y_t;
            end
        end
    ...
    SE_mean(:, :, i_MC) = reshape(SE_mean_par, [H_max+1, 1, n_CI]);
    df_mean(:, :, i_MC) = reshape(df_mean_par, [H_max+1, 1, n_CI]);
    SE_proj(:, :, i_MC) = reshape(SE_proj_par, [H_max+1, 1, n_CI]);
    df_proj(:, :, i_MC) = reshape(df_proj_par, [H_max+1, 1, n_CI]);
